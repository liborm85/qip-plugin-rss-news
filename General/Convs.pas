...
uses SysUtils, Classes, Dialogs, cDateTime, uOptions, TextSearch;
...
const
  ...
  function UTF82WideString(S : AnsiString) : WideString;
...

{
  ...
  function GetAuthor(sAuthor: WideString): WideString;

  procedure ExtractEmail(sEmail: WideString; var sName: WideString; var sServer: WideString);
...
begin
  ...
  sHTML := StringReplace(sHTML, '&gt;',   '>',  [rfReplaceAll]);
  sHTML := StringReplace(sHTML, '&lt;',   '<',  [rfReplaceAll]);
  sHTML := StringReplace(sHTML, '&ndash;',   'ñ',  [rfReplaceAll]);

  iPos := 1;
Re:
  sT := FoundStr(sHTML,'&#',';',iPos);
  iPos := iPos + 1;
  if sT<>'' then
  begin
    if ConvStrToInt(sT)<>0 then
    begin
      sHTML := StringReplace(sHTML, '&#'+sT+';', WideChar(ConvStrToInt(sT)),  [rfReplaceAll]);
    end;
...
(* === Conv Odstranit Diakritiku - odstraÚuje h·Ëky a Ë·rky               === *)
function ConvOdstranitDiakritiku(sName: WideString): WideString;
var sSrc,sNew: WideString;
    i: integer;
begin
  sSrc := '·¡ÈÏÃÌÕÛ”˙⁄˘Ÿ˝›Ë»ÔœlæºnÚ“r¯ÿsöätùçzûé‰Îˆ¸ƒÀ÷‹ﬂ';
...
(*
  sSrc := 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789·¡ÈÏÃÌÕÛ”˙⁄˘Ÿ˝›Ë»ÔœlæºnÚ“r¯ÿsöätùçzûé‰Îˆ¸ƒÀ÷‹ﬂ';
...
function GetAuthor(sAuthor: WideString): WideString;
var sName, sEmail, sURI: WideString;
    IdxOf: Integer;
...
begin
  ...
  r:=StrPosE(sEmail,'@',1,False);

  sServer := Copy( sEmail, r+1 );
  sName   := Copy( sEmail, 1, r-1 );
