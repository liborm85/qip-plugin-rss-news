...
function StrToTimeEx(sTime: AnsiString; var Ye : Word; Mo : Word; Da : Word): TDateTime;
begin

  if CopyLeft(sTime,2)='24' then
  begin
    sTime := '01' + Copy(sTime,3);
    Da := Da + 1;
  end;

  Result := StrToTime(CopyLeft(sTime,8))

end;


function ISO8601StringAsDateTime(const D: AnsiString): TDateTime;
var Date, Time{, sTime} : AnsiString;
    Ye, Mo, Da : Word;
    T: TDateTime;
begin
  //// --- EDITED ---
  if D[5]='-' then  // RFC 3339
  begin
    StrSplitAt(UpperCase(D), 'T', Date, Time);
    Ye := Word(StrToInt(CopyLeft(Date, 4)));
    Mo := Word(StrToInt(CopyRange(Date, 6, 7)));
    Da := Word(StrToInt(CopyRange(Date, 9, 10)));
    if (Length(Time)=9) And (CopyRight(Time,1) = 'Z') then
    begin
      T :=StrToTimeEx(CopyLeft(Time,8),Ye,Mo,Da);
      Result := EncodeDate(Ye, Mo, Da) + T{StrToTime(CopyLeft(Time,8))}
    end
    else
    begin
      T := StrToTimeEx(Time,Ye,Mo,Da);
      Result := EncodeDate(Ye, Mo, Da) + T {StrToTime(Time)}
    end;
  end
  else
  //// ------------
...
const
  ...
  RomNumMonthNames : Array[1..12] of AnsiString = (
...
begin
  ...
  For I := 1 to 12 do
    if StrEqualNoCase(RomNumMonthNames[I], S) then
      begin
        Result := I;
        exit;
      end;
...
//howmessage(Inttostr(Year)+#13+ Inttostr(Month)+#13+ Inttostr(Day)+#13+ Inttostr(Hour)+#13+ Inttostr(Minute)+#13+ Inttostr(Second));
  ...
  if (M = 1) or (M = 3) or (M = 5) or (M = 7) or (M = 8) or
     (M = 10) or (M = 12) then  // 31 dnu
    MaxDays := 31
  else if (M = 4) or (M = 6) or (M = 9) or (M = 11) then  // 30 dnu
    MaxDays := 30
  else if M = 2 then // unor
  begin
  ...
  Result := EncodeDate(Word(Y), Word(M), Word(D));
...
begin
  ...
  if (Copy(T,1,1) = '0') or (Copy(T,1,1) = '1') or (Copy(T,1,1) = '2') or
     (Copy(T,1,1) = '3') or (Copy(T,1,1) = '4') or (Copy(T,1,1) = '5') or
     (Copy(T,1,1) = '6') or (Copy(T,1,1) = '7') or (Copy(T,1,1) = '8') or
     (Copy(T,1,1) = '9') then
  begin
    Goto 1;
  end;
...
//showmessage(inttostr(i));
  if I > 0 then
    begin
      ...
      begin
//        showmessage(Inttostr(DOW));
        T := Trim( Copy(T,I + 1) );   // jinak jazyèné názvy
      end;
    end;
{  else
    begin
